...
D3D::D3D(int sampleCount,int sampleQuality)
{
	m_device = 0;
	m_swapChain = 0;
	...
	this->m_sampleQuality = sampleQuality;
}
...
bool D3D::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	D3D10_RASTERIZER_DESC rasterDesc, rasterDesc2;
	...
	D3D10CreateDevice(NULL,D3D10_DRIVER_TYPE_HARDWARE,NULL,0,D3D10_SDK_VERSION,&m_device);
	...
	else
	{
		...
		depthBufferDesc.SampleDesc.Quality = m_sampleQuality;
	}
	...
	depthStencilViewDesc.ViewDimension= (m_sampleQuality == 0) ? D3D10_DSV_DIMENSION_TEXTURE2D : D3D10_DSV_DIMENSION_TEXTURE2DMS;
	...
	rasterDesc.AntialiasedLineEnable = m_sampleCount > 0 ? true : false;
	...
	rasterDesc.MultisampleEnable = m_sampleCount > 0 ? true : false;
	rasterDesc.ScissorEnable = false;
	rasterDesc.SlopeScaledDepthBias = 0.0f;

	//rasterDesc2.AntialiasedLineEnable = false;
	rasterDesc2.AntialiasedLineEnable = m_sampleCount > 0 ? true : false;
	rasterDesc2.CullMode = D3D10_CULL_BACK;
	rasterDesc2.DepthBias = 0;
	rasterDesc2.DepthBiasClamp = 0.0f;
	rasterDesc2.DepthClipEnable = true;
	rasterDesc2.FillMode = D3D10_FILL_WIREFRAME; //WireFrame
	rasterDesc2.FrontCounterClockwise = false;
	//rasterDesc2.MultisampleEnable = false;
	rasterDesc2.MultisampleEnable = m_sampleCount > 0 ? true : false;
	...
	// Create the rasterizer state from the description we just filled out.
	result = m_device->CreateRasterizerState(&rasterDesc2, &m_rasterStateWire);
	if(FAILED(result))
	{
		return false;
	}
...
UINT D3D::GetMultisampleQualityLevels(UINT numSamples,DXGI_FORMAT format)
{
	UINT q=-1;
	HRESULT(m_device->CheckMultisampleQualityLevels(format,numSamples,&q));
	return q-1;
}
