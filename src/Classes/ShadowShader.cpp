...
ShadowShader::ShadowShader()
{
	...
	m_texturePtr = 0;
	m_depthMapTexturePtr = 0; //The shadow map texture gets initialized to null in the class constructor.
...
bool ShadowShader::Initialize(ID3D10Device* device, HWND hwnd)
{
	bool result;


	// Initialize the shader that will be used to draw the triangles.
	result = InitializeShader(device, hwnd, L"resources/shadow.fx");
	if(!result)
	{
		return false;
	}
...
void ShadowShader::Render(ID3D10Device* device, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix,
...
{
	Render(device, indexCount, worldMatrix, viewMatrix, projectionMatrix, lightViewMatrix, lightProjectionMatrix, texture[0], depthMapTexture, lightPosition, ambientColor, diffuseColor);
}

bool ShadowShader::InitializeShader(ID3D10Device* device, HWND hwnd, WCHAR* filename)
{
	HRESULT result;
...
void ShadowShader::SetShaderParameters(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, D3DXMATRIX lightViewMatrix, 
